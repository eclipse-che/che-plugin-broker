...
func TestBroker_ParseVolumeInThePluginContainer(t *testing.T) {
	vscodeMetaContent := `
--- 
apiVersion: v2
type: "VS Code extension"
spec: 
    containers: 
    - 
        image: eclipse/test-image
        name: test-container
        volumes: 
        - 
            mountPath: /testShouldBePersisted
...
`
	...
	expectedParsedMeta := model.PluginMeta{
		Type:       "VS Code extension",
		ID:         "test-no-registry/1.0",
		APIVersion: "v2",
		Spec: model.PluginMetaSpec{
			Extensions: []string{"http://localhost/test.vsix"},
	...
	m.vscodeBroker.On("ProcessPlugin", mock.MatchedBy(func(pluginMeta model.PluginMeta) bool {
		return assert.ObjectsAreEqual(expectedParsedMeta, pluginMeta)
	})).Return(nil)
	...
	m.vscodeBroker.AssertCalled(t, "ProcessPlugin", mock.AnythingOfType("model.PluginMeta"))
}
